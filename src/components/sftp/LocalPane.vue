...
/* 列排序受控（同远程栏）：显示顺序必须与 files.value 一致——Shift 区间
   选择按 files.value 索引取区间，错位就跳选。表格必须设 remote：
   naive-ui 在受控 sortOrder 下仍会内部再排一次（比较结果乘升/降序
   flag），不加 remote 会双重排序，目录优先被反转、键值双重反转 */
