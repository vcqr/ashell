export default {
  title: "SFTP",
  needSession: "Please establish a connection first",
  dragToResize: "Drag to resize",
  dropHint: "Drop to upload to {path}",
  refresh: "Refresh",
  close: "Close",
  openInNewWindow: "Open in new window",
  goUp: "Up one level",
  copyPath: "Copy path",
  newButton: "New",
  uploadButton: "Upload",
  uploadFolderTitle: "Upload entire folder",
  uploadFolderButton: "Upload folder",
  uploadListTitle: "Upload list",
  uploadListButton: "Upload list",
  downloadListTitle: "Download list",
  downloadListButton: "Download list",
  clearCompleted: "Clear completed",
  remotePaneTitle: "Remote",
  clickToEditPath: "Click the blank area to type a path",
  filterPlaceholder: "Filter current directory…",
  nav: {
    back: "Back",
    forward: "Forward",
  },
  bookmarks: {
    title: "Bookmarks & Recent",
    add: "Bookmark current directory",
    remove: "Remove bookmark for current directory",
    empty: "No bookmarks yet. Click the ☆ next to the address bar to bookmark.",
    recent: "Recent",
    noRecent: "No recent locations",
  },
  localPane: {
    title: "Local",
    thisPc: "This PC",
    goRoots: "This PC (drive list)",
    goHome: "Home directory",
    pathPlaceholder: "Enter an absolute local path (Enter to go)",
    empty: "Directory is empty",
    loadErrorTitle: "Failed to load directory",
    retry: "Retry",
    toggleButton: "Local",
    toggleTitle: "Show/hide local pane (dual-pane mode)",
    close: "Close local pane",
    dragGhost: "{count} item(s) → remote",
    ctxUpload: "Upload this file",
    ctxUploadDir: "Upload this folder",
    ctxUploadMulti: "Upload selected ({count})",
    dropHint: "Drop to copy to {dir}",
    dropNeedsDir: "The local pane is on the This PC page; open a directory first",
    importDone: "Copied {count} file(s) to the local directory",
    importPartial: "Copy finished: {ok} succeeded, {fail} failed",
    loadFailed: "Failed to read local directory: {error}",
    ctxTrash: "Move to Trash",
    trashConfirm:
      "Move {type} \"{name}\" to the Trash? You can restore it from there.",
    trashConfirmMulti:
      "Move these {count} items to the Trash? You can restore them from there.",
    trashed: "Moved to Trash",
    ctxReveal: "Reveal in File Manager",
    ctxOpen: "Open with Default App",
    revealFailed: "Reveal failed: {error}",
    openFailed: "Open failed: {error}",
    deleteConfirm:
      "Permanently delete {type} \"{name}\"? This bypasses the Trash and cannot be undone.",
    deleteConfirmMulti:
      "Permanently delete these {count} items? This bypasses the Trash and cannot be undone.",
  },
  showHidden: "Show hidden files",
  hideHidden: "Hide hidden files",
  elevate: {
    button: "Elevate",
    title: "Rebuild the SFTP session with sudo (root); click again to restore",
    badge: "sudo root",
    badgeTip: "This SFTP session is elevated via sudo; all operations run as root",
    on: "Elevated: the SFTP session now runs as root",
    off: "Restored to a normal SFTP session",
    failed: "Elevate failed: {error}",
    noHost: "Associated host info is missing; cannot elevate",
    passwordTitle: "Sudo password required",
    passwordPlaceholder: "Enter the sudo password of the remote user",
  },
  transferBar: {
    up: "Upload the selected local file(s)/folder(s) to the current remote directory",
    down: "Download the selected remote file(s)/folder(s) to the current local directory",
    dragGhost: "{count} item(s) → local",
  },
  columns: {
    name: "Name",
    size: "Size",
    permission: "Permission",
    userGroup: "User/Group",
    modifyTime: "Modified",
  },
  ctxMenu: {
    newFolder: "New folder",
    newFile: "New file",
    refresh: "Refresh",
    preview: "Preview",
    download: "Download",
    downloadMulti: "Download selected ({count})",
    downloadDir: "Download this folder",
    edit: "Edit",
    rename: "Rename",
    delete: "Delete",
    copy: "Copy",
    copyMulti: "Copy selected ({count})",
    cut: "Cut",
    cutMulti: "Cut selected ({count})",
    paste: "Paste",
    pasteMulti: "Paste ({count} items)",
    selectAll: "Select All",
    copyPath: "Copy path",
    sendToAi: "Send to AI assistant",
    properties: "Properties",
    openTerminalHere: "Open terminal here",
    calcSize: "Calculate directory size",
  },
  newMenu: {
    newFolder: "New folder",
    newFile: "New file",
  },
  properties: {
    title: "Properties",
    name: "Name: ",
    path: "Path: ",
    type: "Type: ",
    size: "Size: ",
    permission: "Permission: ",
    userGroup: "User/Group: ",
    modifyTime: "Modified: ",
    confirm: "OK",
  },
  props: {
    saved: "Properties updated",
    saveFailed: "Update failed: {error}",
    permTitle: "Permissions",
    read: "Read",
    write: "Write",
    exec: "Execute",
    owner: "Owner",
    groupRole: "Group",
    others: "Others",
    octalLabel: "Octal",
    ownerName: "User",
    groupName: "Group",
    nameOrIdHint:
      "Accepts a name or numeric ID; user/group are only submitted when changed.",
    apply: "Apply",
  },
  transfer: {
    eta: "{time} left",
    retry: "Retry",
  },
  message: {
    copied: "Copied",
    copyFailed: "Copy failed",
    loadFailed: "Load failed: {error}",
    folderCreated: "Folder created",
    fileCreated: "File created",
    createFailed: "Create failed: {error}",
    renameSuccess: "Renamed successfully",
    renameFailed: "Rename failed: {error}",
    deleted: "Deleted",
    deleteFailed: "Delete failed: {error}",
    deletePartial: "Partially deleted: {ok} succeeded, {fail} failed",
    downloadOnlyFile: "Only regular files can be downloaded",
    downloaded: "Downloaded: {name}",
    downloadedTo: "Downloaded to local: {dir}",
    downloadFailed: "Download failed: {error}",
    uploaded: "Uploaded: {name}",
    uploadFailed: "Upload failed: {error}",
    uploadedMulti: "Uploaded {count} file(s)",
    uploadFailedMulti:
      "{count} file(s) failed to upload; see the task list for details",
    downloadedMulti: "Downloaded {count} file(s)",
    downloadedToMulti: "Downloaded {count} file(s) to the local directory",
    downloadFailedMulti:
      "{count} file(s) failed to download; see the task list for details",
    noFileSelected: "No file selected",
    cannotParseUploadDir: "Cannot parse upload directory",
    dirUploadDone: "Folder upload complete: {count} files",
    dirDownloadDone: "Folder download complete: {count} files",
    dirDownloadPartial: "Folder download complete: {ok} succeeded, {fail} failed",
    dirUploadPartial: "Folder upload complete: {ok} succeeded, {fail} failed",
    copiedToClipboard: "Copied {count} item(s)",
    cutToClipboard: "Cut {count} item(s)",
    pasted: "Paste completed",
    pasteFailed: "Paste failed: {error}",
    pastePartial: "Partially pasted: {ok} succeeded, {fail} failed",
    dirSize: "{name} uses {size}",
    dirSizeFailed: "Failed to calculate size: {error}",
  },
  dialog: {
    deleteTitle: "Confirm delete",
    deleteConfirm: "Are you sure you want to delete {type} \"{name}\"?",
    deleteConfirmMulti: "Are you sure you want to delete these {count} items?",
    pasteOverwriteTitle: "Overwrite confirmation",
    pasteOverwrite:
      "\"{name}\" already exists here. Overwrite it?",
    pasteOverwriteMulti:
      "{count} item(s) with the same name already exist here. Overwrite them?",
    typeFolder: "folder",
    typeFile: "file",
    typeSymlink: "symlink",
    delete: "Delete",
    cancel: "Cancel",
    downloadTitle: "Download confirm",
    downloadConfirm: "Are you sure you want to download: {name}?",
    downloadConfirmMulti: "Download these {count} files?",
    download: "Download",
    overwriteBatchTitle: "Overwrite {count} same-name items?",
    overwriteTitle: "Overwrite confirm",
    overwriteConfirm: "A file with the same name already exists: {name}. Overwrite?",
    overwrite: "Overwrite",
    uploadOverwriteTitle: "Overwrite confirm",
    uploadOverwriteConfirm: "A remote entry with the same name already exists: \"{name}\". Upload and overwrite all files with the same name?",
  },
  mkdir: {
    newFolder: "New folder",
    newFile: "New file",
    folderPlaceholder: "Enter folder name",
    filePlaceholder: "Enter file name",
    nameRequired: "Name cannot be empty",
    nameContainsSlash: 'Name cannot contain "/"',
    currentDir: "Current directory: {path}",
    cancel: "Cancel",
    confirm: "OK",
  },
  rename: {
    title: "Rename",
    nameRequired: "Name cannot be empty",
    nameContainsSlash: 'Name cannot contain "/"',
    sameName: "New name is the same as the original",
    originalName: "Original name: {name}",
    placeholder: "New name",
    cancel: "Cancel",
    confirm: "OK",
  },
  uploadList: {
    title: "Upload list",
    empty: "No upload tasks",
    cancel: "Cancel",
  },
  downloadList: {
    title: "Download list",
    empty: "No download tasks",
    cancel: "Cancel",
  },
  editor: {
    title: "Edit file",
    saved: "Saved",
    saveFailed: "Save failed: {error}",
    permissionDenied: "No write permission, switched to read-only mode",
    readOnly: "Read Only",
    maximize: "Maximize",
    restore: "Restore",
    binaryFile: "This is a binary file and cannot be edited online",
    fileTooLarge: "File too large ({size}), exceeds edit limit of {max}",
    unsavedTitle: "Unsaved changes",
    unsavedConfirm: "This file has unsaved changes. Close anyway?",
    discard: "Discard changes",
    wordWrap: "Word wrap",
    fontSizeDecrease: "Decrease font size",
    fontSizeIncrease: "Increase font size",
    lineCol: "Ln {line}, Col {col}",
    totalLines: "{count} lines",
  },
  preview: {
    title: "Preview",
    fileTooLarge: "File too large ({size}), exceeds preview limit of {max}",
    unsupportedFormat: "Browser does not support previewing this format, please download to view",
    maximize: "Maximize",
    restore: "Restore",
  },
} as const
